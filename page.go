...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .selector-table-head { display: grid; grid-template-columns: 180px minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1fr) 120px; gap: 12px; padding: 9px 20px 7px; color: #778984; background: #f7faf8; font-size: 11px; font-weight: 700; text-transform: uppercase; }
    .selector-list { display: grid; gap: 8px; padding: 10px; background: #f7faf8; }
    .selector-row { display: grid; grid-template-columns: 180px minmax(0, 1fr) minmax(0, 1fr) minmax(0, 1fr) 120px; gap: 12px; align-items: start; padding: 12px; background: #fff; border: 1px solid #dce5e1; border-radius: 6px; }
    ...
    .selector-match { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) 34px; gap: 6px; align-items: center; }
    .selector-match > :nth-child(1) { grid-column: 1 / 3; grid-row: 1; min-width: 0; }
    .selector-match > :nth-child(2) { grid-column: 1; grid-row: 2; min-width: 0; }
    .selector-match > :nth-child(3) { grid-column: 2; grid-row: 2; min-width: 0; }
    .selector-match > :nth-child(4) { grid-column: 3; grid-row: 2; }
    .selector-rewrite > :nth-child(1) { grid-column: 1 / 3; grid-row: 1; min-width: 0; }
    .selector-rewrite > :nth-child(2) { grid-column: 1 / 3; grid-row: 2; min-width: 0; }
    .selector-rewrite > :nth-child(3) { grid-column: 3; grid-row: 1; }
    .selector-rewrite > :nth-child(4) { grid-column: 3; grid-row: 2; }
...
<body>
  <main class="shell">
    ...
    <section class="workspace selector-workspace" aria-labelledby="selector-config-title">
      <div class="workspace-top">
        <div><h2 class="section-title" id="selector-config-title">AppSelector registry</h2><p class="section-note">按顺序匹配 request header 与 JSON body 字段；首个命中的 selector 决定后续 upstream retry 链。</p></div>
  ...
  <script>
    ...
    function selectorBodyNoRules() { return '<div class="selector-no-rules"><span>No body rules</span><button class="icon-button" type="button" data-add-body-match title="新增 body 条件" aria-label="新增 body 条件"><i data-lucide="plus"></i></button></div>'; }
    function selectorBodyMatchRow() { return '<div class="selector-match" data-selector-body-match data-case-sensitive="false"><input class="field-input" data-body-field value="" placeholder="model" aria-label="JSON 字段"><select class="auth-select" data-match-operator aria-label="匹配方式"><option value="exact" selected>exact</option><option value="prefix">prefix</option><option value="contains">contains</option><option value="regex">regex</option><option value="present">present</option></select><span class="match-value-field"><input class="field-input" data-match-value value="" placeholder="匹配值" aria-label="匹配值"><span class="match-value-actions"><button class="icon-button match-case-toggle" type="button" data-toggle-case title="区分大小写" aria-label="区分大小写" aria-pressed="false"><i data-lucide="case-sensitive"></i></button><button class="icon-button rule-clear" type="button" data-delete-body-match title="删除 body 条件" aria-label="删除 body 条件"><i data-lucide="x"></i></button></span></span><button class="icon-button" type="button" data-add-body-match title="新增 body 条件" aria-label="新增 body 条件"><i data-lucide="plus"></i></button></div>'; }
    function selectorRewriteNoRules() { return '<div class="selector-no-rules"><span>No rewrites</span><button class="icon-button" type="button" data-add-rewrite title="新增 rewrite" aria-label="新增 rewrite"><i data-lucide="plus"></i></button></div>'; }
    function selectorRewriteRow() { return '<div class="selector-match selector-rewrite" data-selector-rewrite><input class="field-input" data-rewrite-field value="" placeholder="model" aria-label="要重写的字段"><input class="field-input" data-rewrite-value value="" placeholder="gpt-5.6-luna" aria-label="重写后的值"><button class="icon-button rule-clear" type="button" data-delete-rewrite title="删除 rewrite" aria-label="删除 rewrite"><i data-lucide="x"></i></button><button class="icon-button" type="button" data-add-rewrite title="新增 rewrite" aria-label="新增 rewrite"><i data-lucide="plus"></i></button></div>'; }
    function selectorRow() { return '<div class="selector-row" data-selector data-draggable draggable="true"><div class="selector-name-cell"><span class="drag-handle" title="拖动排序"><i data-lucide="grip-vertical"></i></span><input class="field-input" data-selector-name value="" placeholder="selector name" aria-label="AppSelector 名称"></div><div class="selector-matches" data-selector-matches>' + selectorNoRules() + '</div><div class="selector-matches selector-body-matches" data-selector-body-matches>' + selectorBodyNoRules() + '</div><div class="selector-matches selector-rewrites" data-selector-rewrites>' + selectorRewriteNoRules() + '</div><div class="selector-actions"><button class="icon-button danger" type="button" data-delete-selector title="删除 AppSelector" aria-label="删除 AppSelector"><i data-lucide="trash-2"></i></button></div></div>'; }
    ...
    document.addEventListener('change', function (event) { const operator = event.target.closest('[data-match-operator]'); if (operator) syncMatchValue(operator.closest('[data-selector-match], [data-selector-body-match]')); });
    saveButton.addEventListener('click', async function () {
      ...
      const appSelectors = [...document.querySelectorAll('[data-selector]')].map(row => ({name: row.querySelector('[data-selector-name]').value.trim(), match: {headers: [...row.querySelectorAll('[data-selector-match]')].map(match => ({name: match.querySelector('[data-match-header]').value.trim(), operator: match.querySelector('[data-match-operator]').value, value: match.querySelector('[data-match-value]').value, caseSensitive: match.dataset.caseSensitive === 'true'})), body: [...row.querySelectorAll('[data-selector-body-match]')].map(match => ({field: match.querySelector('[data-body-field]').value.trim(), operator: match.querySelector('[data-match-operator]').value, value: match.querySelector('[data-match-value]').value, caseSensitive: match.dataset.caseSensitive === 'true'}))}, rewrite: [...row.querySelectorAll('[data-selector-rewrite]')].map(rewrite => ({field: rewrite.querySelector('[data-rewrite-field]').value.trim(), value: rewrite.querySelector('[data-rewrite-value]').value}))}));
    ...
    document.addEventListener('click', function (event) {
      ...
      if (caseToggle) { const match = caseToggle.closest('[data-selector-match], [data-selector-body-match]'); const enabled = match.dataset.caseSensitive !== 'true'; match.dataset.caseSensitive = String(enabled); caseToggle.classList.toggle('is-active', enabled); caseToggle.setAttribute('aria-pressed', String(enabled)); caseToggle.title = enabled ? '区分大小写：开' : '区分大小写：关'; caseToggle.setAttribute('aria-label', caseToggle.title); return; }
      ...
      const addBodyMatch = event.target.closest('[data-add-body-match]');
      if (addBodyMatch) { const matches = addBodyMatch.closest('[data-selector]').querySelector('[data-selector-body-matches]'); const empty = matches.querySelector('.selector-no-rules'); if (empty) empty.remove(); matches.insertAdjacentHTML('beforeend', selectorBodyMatchRow()); renderIcons(matches); return; }
      const deleteBodyMatch = event.target.closest('[data-delete-body-match]');
      if (deleteBodyMatch) { const matches = deleteBodyMatch.closest('[data-selector-body-matches]'); deleteBodyMatch.closest('[data-selector-body-match]').remove(); if (!matches.querySelector('[data-selector-body-match]')) matches.insertAdjacentHTML('beforeend', selectorBodyNoRules()); renderIcons(matches); return; }
      const addRewrite = event.target.closest('[data-add-rewrite]');
      if (addRewrite) { const matches = addRewrite.closest('[data-selector]').querySelector('[data-selector-rewrites]'); const empty = matches.querySelector('.selector-no-rules'); if (empty) empty.remove(); matches.insertAdjacentHTML('beforeend', selectorRewriteRow()); renderIcons(matches); return; }
      const deleteRewrite = event.target.closest('[data-delete-rewrite]');
      if (deleteRewrite) { const matches = deleteRewrite.closest('[data-selector-rewrites]'); deleteRewrite.closest('[data-selector-rewrite]').remove(); if (!matches.querySelector('[data-selector-rewrite]')) matches.insertAdjacentHTML('beforeend', selectorRewriteNoRules()); renderIcons(matches); return; }
